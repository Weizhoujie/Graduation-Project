...
<body>
	...
	<form id="form1" name="Form1" onsubmit="return checkdata()" action="${pageContext.request.contextPath}/QueryStuByUsernameServlet" method="post">
		...
		<input type="button" style="margin-left: 930px;" class="btn btn-info btn-xs" onclick="tiaozhuan()" value="新增申请">
		<br/>
						...
						<table cellspacing="0"  cellpadding="1" rules="all"
							...
							<c:forEach items="${allapplication }" var="leaveapp" varStatus="vs">
								...
								<td style="CURSOR: hand; HEIGHT: 22px" align="center"
									...
									<c:choose>
										<c:when test="${leaveapp.status eq '1'}">
											<td style="CURSOR: hand; HEIGHT: 22px;" align="center"
												width="4%">已通过</td>
										</c:when>
										<c:when test="${leaveapp.status eq '0'}">
											<td style="CURSOR: hand; HEIGHT: 22px;color: green;" align="center"
												width="4%">待审批</td>
										</c:when>
										<c:otherwise>
									...
									<td align="center" style="HEIGHT: 4px">
											<a href="BatchDelLeaveAppServlet?delleaveappid=${leaveapp.id}"><input type="button" id="delUser" class="btn btn-info btn-xs" onclick="return delCheck()" value="删除"></a>
									</td>
		...
		<ul class="pagination" style="text-align: center; margin-top: 10px;">
			...
			<c:if test="${pageBean.currentPage!=1 }">
				<li>
					<a href="${pageContext.request.contextPath }/AllLeaveAppByStuServlet?page=${pageBean.currentPage-1}" aria-label="Previous">
						 <span aria-hidden="true">&laquo;</span>
					</a>
				</li>
			</c:if>
			...
			<c:forEach begin="1" end="${pageBean.totalPage }" var="page">
				...
				<c:if test="${pageBean.currentPage!=page }">
				<li><a href="${pageContext.request.contextPath }/AllLeaveAppByStuServlet?page=${page}">${page }</a></li>
				</c:if>
			</c:forEach>
			...
			<c:if test="${pageBean.totalPage!=pageBean.currentPage }">
				<li>
					<a href="${pageContext.request.contextPath }/AllLeaveAppByStuServlet?page=${pageBean.currentPage+1}" aria-label="Next"> <span aria-hidden="true">&raquo;</span></a>
				</li>
			</c:if>
		</ul>
	</div>
...
function check() {
    ...
    if (confirm(msg)==true){   
        ...
        if(becheckbox.length > 0){
            ...
            window.location = "${pageContext.request.contextPath }/BatchDelLeaveAppServlet?delleaveappid="+becheckbox;
        }else {
			alert("请选择需要删除的内容！");
			return false;
		}
...
function tiaozhuan(){ 
	window.location.href = "${pageContext.request.contextPath }/CourseAndTeaBeforeLeaveServlet";
}
