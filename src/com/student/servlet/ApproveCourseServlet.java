...
public class ApproveCourseServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		 ...
		 for (StudentChoseCourse studentChoseCourse : list1) {
			if (studentChoseCourse.getFlag() == 0) {
				list2.add(studentChoseCourse);
			}
		}
		 request.setAttribute("approvecourses", list2);
		 request.getRequestDispatcher("/page/student/approvecourse.jsp").forward(request, response);
	}
