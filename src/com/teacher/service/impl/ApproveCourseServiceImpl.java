...
import com.teacher.dao.impl.ApproveCourseDaoImpl;
import com.teacher.entity.PendingCourse;
import com.teacher.service.ApproveCourseService;

public class ApproveCourseServiceImpl implements ApproveCourseService{
	ApproveCourseDao dao = new ApproveCourseDaoImpl();

	@Override
	public List<PendingCourse> queryAllPendingCourse(long tid, int startIndex, int currentCount) {
		List<PendingCourse> list = dao.queryAllPendingCourse(tid, startIndex, currentCount);
		return list;
	}

	@Override
	public PageBean<PendingCourse> findPageBean(long tid,int currentPage) {
		PageBean<PendingCourse> pageBean = new PageBean<PendingCourse>();
		//1.当前页数		---- 前台用户点击的页数
		...
		List<PendingCourse> courses = dao.queryAllPendingCourse(tid, startIndex, currentCount);
		pageBean.setPendingCourses(courses);
		return pageBean;
	}

	@Override
	public int batchApproveCourse(String s) {
		int result = dao.batchApproveCourse(s);
		return result;
	}

	@Override
	public int rejectCourse(long id) {
		int result = dao.rejectCourse(id);
		return result;
	}
